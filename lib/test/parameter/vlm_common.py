...
yaml_name 后缀约定 (qwen_vlm_visual / qwen_vlm_hybrid):
    {model_key}[_{N}f]        -- N帧, e.g. default_2f / local_3b_3f
    {model_key}_{vlm}_{trig}  -- hybrid专用, e.g. default_visual_kf
"""
import os

_QWEN25_ROOT = '/data/MODEL_WEIGHTS_PUBLIC/MLLM_weights'
...
MODEL_CONFIGS = {
    'local_3b': {
        ...
        'model_path': os.path.join(_QWEN25_ROOT, 'Qwen2.5-VL-3B-Instruct'),
    },
    'local_7b': {
        'mode': 'local',
        'model_name': 'Qwen/Qwen2.5-VL-7B-Instruct',
        'model_path': os.path.join(_QWEN25_ROOT, 'Qwen2.5-VL-7B-Instruct'),
    },
    # 兼容旧命名，避免现有 debug/脚本失效
    'local_4b': {
        'mode': 'local',
        'model_name': 'Qwen/Qwen2.5-VL-3B-Instruct',
        'model_path': os.path.join(_QWEN25_ROOT, 'Qwen2.5-VL-3B-Instruct'),
...
def parse_yaml_name(yaml_name: str):
    ...
    qwen_vlm_visual 示例:
        ...
        'local_3b_3f'   → ('local_3b', {'num_frames': 3})
    ...
    qwen_vlm_hybrid 示例:
        ...
        'local_3b_visual_hybrid'  → ('local_3b', {'vlm_mode': 'visual',   'trigger_mode': 'hybrid'})
    """
...
def apply_vlm_config(params, yaml_name: str):
    ...
    if params.mode == 'local':
        params.model_name = config.get('model_name', 'Qwen/Qwen2.5-VL-3B-Instruct')
